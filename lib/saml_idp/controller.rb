  ...
  module Controller
    ...
    protected
      ...
      def decode_SAMLRequest(saml_request)
        saml_request = CGI.unescape(saml_request)
      ...
      def encode_SAMLResponse(nameID, opts = {})
        ...
        doc = Nokogiri.parse(signed_info)
        canon_algorithm = Nokogiri::XML::XML_C14N_EXCLUSIVE_1_0
        canon_signed_info = doc.canonicalize(canon_algorithm)
        signature_value = sign(signed_info).gsub(/\n/, '')

        signature = %[<ds:Signature xmlns:ds="http://www.w3.org/2000/09/xmldsig#">#{canon_signed_info}<ds:SignatureValue>#{signature_value}</ds:SignatureValue><ds:KeyInfo><ds:X509Data><ds:X509Certificate>#{self.x509_certificate}</ds:X509Certificate></ds:X509Data></ds:KeyInfo></ds:Signature>]

        assertion_and_signature = assertion.sub(/Issuer\>\<(saml2:)?Subject/) { |s| "Issuer>#{signature}<#{$1}Subject" }

        xml = %[<saml2p:Response ID="_#{response_id}" Version="2.0" IssueInstant="#{now.iso8601}" Destination="#{@saml_acs_url}" Consent="urn:oasis:names:tc:SAML:2.0:consent:unspecified" InResponseTo="#{@saml_request_id}" xmlns:saml2p="urn:oasis:names:tc:SAML:2.0:protocol"><Issuer xmlns="urn:oasis:names:tc:SAML:2.0:assertion">#{issuer_uri}</Issuer><saml2p:Status><saml2p:StatusCode Value="urn:oasis:names:tc:SAML:2.0:status:Success" /></saml2p:Status>#{assertion_and_signature}</saml2p:Response>]
